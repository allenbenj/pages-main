...
files.forEach(f => {
  ...
  const m = content.match(/(href|src|url|data-href)\s*=\s*['"]?(\.\.[^'"\s>]+)['"]?/gi);
  if (m) {
     m.forEach(match => {
...
console.log('Found ' + externalReferences.length + ' external refs with ../');
externalReferences.slice(0, 20).forEach(o => console.log(o.file, o.match));
